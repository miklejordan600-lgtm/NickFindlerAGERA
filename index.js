...
/* ================== КРИТИЧЕСКИЕ ИСПРАВЛЕНИЯ ДЛЯ RAILWAY ================== */

// 1. HTTP-сервер "заглушка" ( Railway прибивает процесс, если порт не активен )
import http from "http";
try {
  http.createServer((req, res) => {
    res.writeHead(200, { 'Content-Type': 'text/plain' });
    res.end('Minecraft Monitor is active');
  }).listen(process.env.PORT || 3000, () => {
    console.log(`[SYSTEM] Web-server listening on port ${process.env.PORT || 3000}`);
  });
} catch (e) {
  console.error("[SYSTEM] Failed to start HTTP server:", e);
}

// 2. Глобальный перехват ошибок (защита от падения 24/7)
...
// 3. Единая точка запуска (сначала ТГ, потом Майн)
(async () => {
  try {
    console.log("[SYSTEM] Starting services...");
    // Запуск Телеграм с твоей функцией защиты от 409 ошибки
